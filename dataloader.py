...
class loaddataset(Dataset):
    def __init__(self, datadir= maindir  + '/Training_Samples/Train'):
...
if __name__ == '__main__':
    # Test Dataloader
    Path(maindir + '/output').mkdir(parents=True, exist_ok=True)
    dataset = loaddataset(maindir  + '/Database/Training_Samples/Train')
    ...
    for i, batch in enumerate(dataloader):
        if i == 10:
            ...
            mpimg.imsave(maindir  + '/output/noisy_mag.png', np.flipud(nspec), cmap='jet')
            mpimg.imsave(maindir  + '/output/clean_mag.png', np.flipud(cspec), cmap='jet')
            break
