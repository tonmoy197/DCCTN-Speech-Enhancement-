...
def datagenerator(In_path, Out_path, Noise_file, SNR, Num_audio_sample, sample_margin, Fs):
    for index in range(Num_audio_sample):
        ...
        clean, Fs = lr.load(In_path + '/Clean/Clean_SPK1_S' + str(index + sample_margin) + '.wav', sr=Fs)  # Downscale 48kHz to 16kHz
        noise, Fs = lr.load(In_path + '/Noise/' + Noise_file + '.wav', sr=Fs)  # Downscale 48kHz to 16kHz
...
if __name__ == '__main__':
    ...
    Num_audio_sample = 10
    sample_margin=1
    In_path = 'F:\Speech Enhancement Projects\DNS-Chellenge'
    for Noise_file in tqdm(Noise_type, desc="Generating Train data "):
    # for Noise_file in Noise_type:
        for SNR in SNR_all:
            datagenerator(In_path, Data_path + '/Train/', Noise_file, SNR, Num_audio_sample,sample_margin, Fs)
    ...
    for Noise_file in tqdm(Noise_type, desc="Generating Validation data "):
        for SNR in SNR_all:
            datagenerator(In_path, Data_path + '/Dev/', Noise_file, SNR, Num_audio_sample,sample_margin, Fs)
    ...
    SNR_all = [5, 10]
    for Noise_file in tqdm(Noise_type, desc="Generating Testing data "):
        for SNR in SNR_all:
            datagenerator(In_path, Data_path + '/Test/', Noise_file, SNR, Num_audio_sample, sample_margin, Fs)
